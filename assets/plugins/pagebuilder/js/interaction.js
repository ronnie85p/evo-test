  ...
  return (function ($) {
    var ContentBlock = {
        ...
        $(blocks).each(function () {
          ...
          $block.find(".sortable-list").each(function () {
            ...
            if ($fields.children(".type-image").length) {
              $(
                '<input type="button" class="btn btn-secondary fill-with" data-type="image">'
              )
                .val(opts.lang["Fill with images"])
                .appendTo($buttons);
            }

            if ($fields.children(".type-file").length) {
              $(
                '<input type="button" class="btn btn-secondary fill-with" data-type="file">'
              )
                .val(opts.lang["Fill with files"])
                .appendTo($buttons);
            }
          ...
          $block.on("click", ".open-browser", function (e) {
            e.preventDefault();
            ContentBlock.openBrowser(
              $(this).next("input"),
              $(this).parent().hasClass("type-image") ? "images" : "files"
            );
          });
          ...
          $block.on("click", "[data-toggle=collapse]", (e) => {
            const $target = $($(e.target).data("target"));

            $target.toggleClass("show");
          });

          $block.on("input", ".field[data-field=icon] input", (e) => {
            let icon;
            try {
              icon = $(e.target.value).get(0);
            } catch (e) {}
          ...
          $block.on("click", ".field[data-field=icon] .icons i", (e) => {
            const iconHTML = e.target.outerHTML;
            const $field = $(e.target).closest(".field");

            $field.find("input").val(iconHTML);
            $field.find(".icon-prev").html(iconHTML);
            $field.find(".icons i").removeClass("active");
          ...
          switch (conf[field].type) {
            ...
            case "group": {
              ...
              $field
                ...
                .children(".sortable-item:not(.hidden)")
                .each(function (i) {
            ...
            case "icon":
              values[field] = $field.find("input").val();
              break;

            default: {
              values[field] = $field
                .children('input[type="text"], textarea, select')
                .val();
            }
          }
        }
            ...
            case "group": {
              if (typeof values[field] === "object" && values[field].length) {
                ...
                for (var i = 0; i < values[field].length; i++) {
                  ...
                  ContentBlock.setValues(
                    conf[field].fields,
                    $clone.children(".fields-list"),
                    values[field][i]
                  );
                }
              }
      ...
      changeType: function ($block, type) {
        var values = this.fetch($block, $block.data("values")),
          ...
          $newblock = $('.content-blocks-configs[data-formid="' + formid + '"]')
            .children('[data-config="' + type + '"]')
            .eq(0);
        ...
        if ($newblock.length) {
          ...
          $newblock
            ...
            .replaceWith(
              $block.children(".block-inner").children(".change-type")
            );
          $block.replaceWith($newblock);

          ContentBlock.setValues(
            opts.config[type].fields,
            $newblock.children(".block-inner").children(".fields-list"),
            values
          );
      ...
      append: function (config, $after) {
        var formid = $after.closest(".content-blocks").attr("data-formid"),
          $block = $('.content-blocks-configs[data-formid="' + formid + '"]')
            .children('[data-config="' + config + '"]')
            .eq(0);
        ...
        if ($block.length) {
          ...
          $block.find(".type-radio, .type-imageradio").each(function () {
            $(this)
              ...
              .attr(
                "name",
                "contentblocks_radio_" + ContentBlock.randomString()
              );
          });
      ...
      createSortableItem: function ($list) {
        ...
        $clone
          .children(".fields-list")
          .children(".type-radio, .type-imageradio")
          .each(function () {
            $(this)
              .find('[type="radio"]')
              .attr(
      ...
      insertItem: function ($after) {
        ...
        ContentBlock.groupUpdated($list);
        ContentBlock.initializeFieldsList(
          $after.children(".fields-list").data("fields"),
          $clone.children(".fields-list")
        );
        ...
        for (var field in conf) {
          ...
          switch (fieldConfig.type) {
            ...
            case "image": {
              ...
              $input.attr(
                "id",
                "fm" + ContentBlock.randomString() + ContentBlock.randomString()
              );
      ...
      initializeRichField: function ($textarea) {
        if (!$textarea.is("TEXTAREA") || $textarea.closest(".hidden").length) {
          return;
        }
        ...
        if (typeof tinymce !== "undefined") {
          var conf =
            theme != undefined
              ? window["config_tinymce4_" + theme]
              : window[modxRTEbridge_tinymce4.default];
      ...
      openBrowser: function ($element, type, multipleCallback) {
        var wnd = window.parent || window,
          ...
          params =
            "toolbar=no,status=no,resizable=yes,dependent=yes,width=" +
            width +
            ",height=" +
            height +
            ",left=" +
            margin +
            ",top=" +
            (margin + (wnd._startY ? wnd._startY * 0.5 : 0));
    ...
    opts.containers.each(function () {
      ...
      if ($container.hasClass("single")) {
        $container.on(
          "click",
          ".dropdown-add-block, .add-block .trigger a",
          function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        var $a = $("<a>")
          .attr(
            ...
            "data:text/json;charset=utf-8," +
              encodeURIComponent(JSON.stringify(values, "", 2))
          )
          ...
          reader.onload = (function (file, $control) {
            return function (e) {
              ...
              for (var i = json.length - 1; i >= 0; i--) {
                ...
                ContentBlock.setValues(
                  opts.config[json[i].config].fields,
                  $block.find(".fields-list").first(),
                  json[i].values
                );
              }
            };
    ...
    opts.containers
      ...
      .submit(function () {
        ...
        $(".content-blocks").each(function () {
          ...
          $(this)
            ...
            .each(function () {
              var $block = $(this),
                ...
                $id = $inner.find('[name="contentblocks_id"]'),
                data = {
                  visible: $inner
                    .find(".change-type")
                    .find(".visible")
          ...
          if (!length) {
            $('<input type="hidden"/>')
              .attr("name", "contentblocks[" + container + "]")
              .val("0")
              .appendTo($form);
          }
        });
